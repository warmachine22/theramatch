(function (global) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Search legend with per-case distance from current child
    const renderSearchLegendForTherapist = async (therapist) => {
      ...
      // 1) Prefer referral by Child ID (validated & saved coords)
      // 2) Fallback to geocoding Search form fields
      let childCoord = null;
      try {
        const childId = (searchChildIdInput && (searchChildIdInput.value || '').trim()) || '';
        if (childId) {
          const ref = (TMS.Store.findReferralByChildId && TMS.Store.findReferralByChildId(childId)) || null;
        ...
        if (!childCoord) {
          ...
          if (cs || city || st || zip) {
            try {
              const gc = await geocodeAddressCached({ address: '', crossStreets: cs, city, state: st, zip });
      ...
      if (changed) {
        therapists = Store.setTherapists((prev) => prev.map((x) => x.id === therapist.id ? therapist : x));
        if (currentSelectedTherapist && currentSelectedTherapist.id === therapist.id) {
          currentSelectedTherapist = therapists.find((t) => t.id === therapist.id);
        }
      }

      // Build legend markup with hours and distance
      searchCasesLegend.innerHTML = '';
      const cases = therapist.cases || [];
      ...
      cases.forEach((c) => {
        ...
        if (childCoord && typeof c.lat === 'number' && typeof c.lon === 'number') {
          const d = haversineMiles(childCoord.lat, childCoord.lon, c.lat, c.lon);
      ...
      searchCasesLegend.appendChild(frag);
    };
    ...
    const loadReferralToSearch = (ref) => {
      ...
      searchSelectedSlots = new Set(slots);
    ...
    function buildQueensDataset() {
      ...
      const therapists = staffedDefs.map((def, idx) => {
      ...
      const unstaffed = [
        {
          ...
          // Mon–Fri 15:00–16:00 = 5.0h total
        ...
        },
        {
          ...
          // Mon/Wed/Fri 14:00–15:30 (1.5h ×3 = 4.5h) + Tue/Thu 09:00–10:00 (1h ×2 = 2h) = 6.5h
          blocks: [
        ...
        },
        {
          ...
          // Mon–Fri 10:00–11:15 (1.25h ×5 = 6.25h)
